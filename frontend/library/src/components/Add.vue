...
<template>
    ...
    <div id="idDivclearAdd">
        ...
        <p id="kleinerTippAddSeite">Kleiner Tipp, wenn du den genauen Titel und Autor deines Buches kennst können wir den
            Rest teilweise oder ganz autovervollständigen.</p>
        <v-form id="Add" ref="addForm">
            <div id="idBuch">
                <v-text-field v-model="this.title" label="Titel" @blur="getBook()" id="AddTitle"
                    :rules="[v => !!v || 'Title ist ein Pflichtfeld']" type="text" required></v-text-field>
                <div id="idImgDiv">
                    <img :src="this.cover" id="idImg">
                </div>
                <v-text-field v-model="this.author" label="Author" @blur="getBook()" id="AddAuthor"
                    :rules="[v => !!v || 'Autor ist ein Pflichtfeld']" type="text" required></v-text-field>
                <v-text-field v-model="this.genre" label="Genre" id="Genre" :rules="[v => !!v || 'Genre ist ein Pflichtfeld']"
                    type="text" required></v-text-field>
                <v-text-field v-model="this.language" label="Sprache" @blur="this.resolveAbbreviation()" id="Sprache"
                    :rules="[v => !!v || 'Title ist ein Pflichtfeld']" type="text" required></v-text-field>
                <v-text-field v-model="this.isbn" label="ISBN" @blur="getCoverByISBN()" id="ISBN"
                    :rules="[v => !!v || 'ISBN ist ein Pflichtfeld']" type="number" min="0" step="1" required></v-text-field>
                <v-text-field v-model="this.releaseYear" label="Erscheinungsjahr" id="Erscheinungsjahr"
                    :rules="[v => !!v || 'Title ist ein Pflichtfeld']" type="number" min="0" step="1" required></v-text-field>
                <v-textarea v-model="this.description" label="Beschreibung" id="Beschreibung"
                    :rules="[v => !!v || 'Beschreibung ist ein Pflichtfeld']" type="text" required></v-textarea>
                <v-text-field v-model="this.pages" label="Seitenzahl" id="Seitenzahl"
                    :rules="[v => !!v || 'Title ist ein Pflichtfeld']" type="number" min="0" step="1"></v-text-field>
            </div><!-- Buch bis Hier-->

            <div id="idOwner">
                <v-text-field v-model="this.owner.firstName" label="Vorname" id="Sprache"
                    :rules="[v => !!v || 'Vorname ist ein Pflichtfeld']" type="text"></v-text-field>
                <v-text-field v-model="this.owner.lastName" label="Nachname" id="Sprache"
                    :rules="[v => !!v || 'Nachname ist ein Pflichtfeld']" type="text"></v-text-field>
                <v-text-field v-model="this.owner.phoneNumber" label="Handynummer" id="Sprache"
                    :rules="[v => !!v || 'Handynummer ist ein Pflichtfeld']" type="text"></v-text-field>
                <v-text-field v-model="this.owner.eMail" label="E-Mail" id="Sprache"
                    :rules="[v => !!v || 'Email ist ein Pflichtfeld']" type="text"></v-text-field>
            </div><!-- Owner bis Hier-->
...
<style>@import "../assets/add.module.css";</style>